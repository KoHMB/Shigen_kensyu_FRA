...
SRregime_plot(resR11_L1HS, regime.name = c("A","B"))

# 再生産関係にべバートンホルト型を指定、推定方法を最小二乗法とし（method="L2"））、2005年にレジームが切り替わり、b,sdがレジームで変更される場合
resR11_L1HS <- fit.SRregime(SRdata, SR = "BH", method = "L2", regime.year = c(2005), regime.par = c("a","b","sd")[2:3], use.fit.SR = TRUE, regime.key = c(0, 1), bio_par=bio_par)
# 結果をテキストでファイル出力
